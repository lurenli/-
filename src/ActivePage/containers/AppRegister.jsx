...
import ActiveInput from '../../ActivePage/components/ActiveInput';
...
const {Tab, NavBarItem, Cells, Cell, CellHeader, CellBody, Toptips, Dialog} = WeUI;
...
export default class AppRegister extends Component {
    constructor(props) {
        ...
        this.GetReword = this.GetReword.bind(this);
        this.state = {
            phone: this.props.location.query.mobile,
            showWarn: false,
            tipText:"",
            showDialog: false,
            style: {
    ...
    componentDidMount() {
        document.getElementById("phoneNum").value=this.props.location.query.mobile;
    }
    ...
    GetReword() { //领取红包
        ...
        if (!userName) {
            ...
            this.RegisterClick(userName, password, realname, phoneNum, checkCode, invitation);
        }
    }
    //注册
    RegisterClick(userName, password, realname, phoneNum, checkCode, invitation) {
        let fromData = new FormData();
        fromData.append("username", userName);
        fromData.append("password", Base64.encode(password));
        fromData.append("realname", realname);
        ...
        fetch(getHost() + "/rest/memberactivity/joinActivity", {
            ...
            if (json.result === 1) {//success
                ...
                if(json.message==="该号码已参与过此活动，不能重复参与"){
                    ...
                    let url = pathHeader+"/AppRegister?mobile="+phone;
                    location.href = url;
                }
            }
        }).catch(e => {
        ...
        return (
            <div  id="scrollDiv"  style={{width: "100%", height: "100%"}}>
                <div className="AppActive" style={{backgroundColor:'#fecc00'}}>
                    <img src="/assets/images/AppActive/Newregister_bg .jpg" width={bgWidth} />
                    <div className="Active_content">
                        <div style={inputs}>
                            <ActiveInput Id="userName"  Min="3" Max="20"
                                          Placeholder="请输入用户名（3-12位英文，数字）" />
                            <ActiveInput Id="phoneNum"  Placeholder="请输入手机号"/>
                            <ActiveInput Id="checkCode"  Type="check"
                                          Placeholder="请输入验证码"/>
                            <ActiveInput Id="realname"
                                         Placeholder="请输入真实姓名"/>
                            <ActiveInput Id="password"
                                         Placeholder="请输入密码（6-20位）" showType="password"/>
                            <ActiveInput Id="repeat"
                                          Placeholder="请输入确认密码（6-20位）" showType="password"/>
                            <ActiveInput Id="invitation"
                                          Placeholder="请输入邀请码（选填）"/>
                            <div style={{fontSize: "1.4rem",color:"#fff",textAlign:"left"}}>注册即视为同意
                                <span style={{color: "#fff",fontWeight:"bold", textDecoration: "no-underline" ,cursor:"pointer"}}  onClick={e => this.setState({showDialog: true})}>《紧商注册协议》</span>
                            </div>
                        </div>
                        <div>
                            <div className="success_back">
                                <span className="register_span" onClick={this.GetReword}>马上领红包</span>
                            </div>
                        </div>
                    </div>
                </div>
